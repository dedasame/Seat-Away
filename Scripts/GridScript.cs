...
public class GridScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if(doubleSeat) CreateDoubleSeat();
    ...
    public void CreateDoubleSeat()
    {
        seat = Instantiate(GameManager.Instance.doubleSeatPrefab, transform.position, Quaternion.identity);
        seat.GetComponent<SeatScript>().grid = this;
        seat.GetComponent<SeatScript>().canMove = false;
        seat.GetComponent<SeatScript>().color = seatColor;
        seat.GetComponent<SeatScript>().SetColor();
    }
