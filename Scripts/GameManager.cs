...
public class GameManager : MonoBehaviour
{
    ...
    public TextMeshProUGUI timeText; //sureyi yazdiracak
    public float timer; //sure
    ...
    void Update()
    {
        ...
        //timer
        if(timer > 0 && !levelCompleted && timeDown)
        {
            timer = timer - (Time.deltaTime* 1f);
            timeText.text = "Time : " + timer.ToString("0");
        }
        else if(timer <=0 && !levelCompleted && isChecked)
        {
    ...
    public void SettingButton() //ayarlar icin canvas ac
    {
        ...
        timeDown = false; //sureyi durdur    
    }
    public void CloseSettingMenuButton()
    {
        settingScreen.SetActive(false);
        timeDown = true; //sureyi baslat
    }
